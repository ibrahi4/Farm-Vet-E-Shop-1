...
export default function Checkout() {
  ...
  const placeOrder = async () => {
    ...
    if (stockIssue) {
      toast.error(t("checkout.messages.stockIssue"));
      return;
    }
    ...
    try {
      ...
      toast.error(err?.message || t("checkout.messages.failure"));
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <section className="mx-auto mt-20 max-w-2xl rounded-3xl border border-amber-200 bg-amber-50/80 p-10 text-center shadow-sm">
        ...
        <p className="mt-2 text-sm text-amber-800">
          {t("checkout.loginRequired.subtitle")}
        </p>
        <div className="mt-6 flex justify-center">
          <Link
            to="/login"
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            ...
            >
              {loading
                ...
                : t("checkout.actions.confirmOrder")}
            </button>
          </div>
        </form>
