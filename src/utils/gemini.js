// src/utils/gemini.js
import { GoogleGenerativeAI } from "@google/generative-ai";

const genAI = new GoogleGenerativeAI("AIzaSyCx2Ozccf32VAv1nmqECMN8ATTnHxK94SM");

const model = genAI.getGenerativeModel({
  model: "gemini-2.5-flash", // غيّر لـ "gemini-1.5-pro-latest" لو عايز قوة أكبر (لسه مجاني)
  generationConfig: {
    temperature: 0.7,
    topP: 0.95,
    topK: 40,
    maxOutputTokens: 8192,
    responseMimeType: "text/plain",
  },
});

export const analyzeWithGemini = async (firebaseData, customPrompt = null) => {
  const defaultPrompt = `
أنت خبير تحليل بيانات ومحلل أعمال متقدم وبتتكلم عربي ممتاز.

البيانات دي جاية من Firebase دلوقتي:
${JSON.stringify(firebaseData, null, 2)}

اعمل تحليل ذكي ومتقدم بالعربي الفصحى ويكون فيه:

## 📊 الملخص التنفيذي
- نظرة سريعة على الأداء الحالي في 3-4 جمل
- أهم النقاط اللي تحتاج انتباه فوري

## 📈 التحليل التفصيلي
### إحصائيات أساسية:
- إجمالي الطلبات والمنتجات والمستخدمين
- إجمالي المبيعات والمتوسط
- أعلى المنتجات مبيعاً
- أكثر العملاء شراءاً

### أنماط واتجاهات:
- تحليل سلوك العملاء
- التقلبات الموسمية
- المنتجات بطيئة الحركة
- نقاط القوة والضعف

## ⚠️ التحذيرات المهمة
- منتجات قريبة من النفاد
- أنماط مشبوهة في الطلبات
- مشاكل محتملة في المخزون

## 🎯 التوصيات الذكية
5 توصيات عملية ومحددة:
1. قرارات فورية (خلال 24 ساعة)
2. تحسينات متوسطة المدى (أسبوع)
3. استراتيجيات طويلة المدى (شهر)

## 📋 خطة عمل مقترحة
خطوات محددة ومجدولة بتنفيذ عملي

استخدم:
- جداول Markdown للبيانات الكثيرة
- Emojis لتسهيل القراءة
- تنسيق جميل ومنظم
- أرقام محددة وليست عامة
`;

  try {
    const result = await model.generateContent(customPrompt || defaultPrompt);
    const response = await result.response;
    const text = response.text();
    return text;
  } catch (error) {
    console.error("خطأ في Gemini:", error);

    
    return `عذراً، حصل مشكلة في الاتصال بالذكاء الاصطناعي.\nالتفاصيل: ${error.message}\n\nجرب تاني بعد شوية أو تأكد إن النت شغال كويس.`;
  }
};

// اختبار سريع (اختياري - امسح السطر ده لو مش عايزه)
// console.log("Gemini جاهز للتحليل يا وحش!");
